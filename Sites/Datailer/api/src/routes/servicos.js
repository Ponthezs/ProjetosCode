...
router.get('/:lavacarId', async (req, res) => {
  try {
    const list = await prisma.servico.findMany({ where: { lavaCarId: req.params.lavacarId, ativo: true } });
    res.json(list);
  } catch (e) {
    console.error(e);
...
router.post('/', async (req, res) => {
  try {
    ...
    if (!lavaCarId || user.id !== lavaCarId || !nome || preco == null) return res.status(400).json({ erro: 'lavaCarId, nome e preco obrigatórios' });
    const s = await prisma.servico.create({
      data: { lavaCarId, nome: (nome || '').trim(), preco: parseFloat(preco) || 0, duracao: parseInt(duracao, 10) || 60, descricao: (descricao || '').trim() || null },
    });
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const updated = await prisma.servico.update({
      ...
      data: { ...(nome != null && { nome: String(nome).trim() }), ...(preco != null && { preco: parseFloat(preco) || 0 }), ...(duracao != null && { duracao: parseInt(duracao, 10) || 60 }), ...(descricao != null && { descricao: String(descricao).trim() || null }), ...(ativo != null && { ativo: !!ativo }) },
    });
    res.json(updated);
  } catch (e) {
    console.error(e);
