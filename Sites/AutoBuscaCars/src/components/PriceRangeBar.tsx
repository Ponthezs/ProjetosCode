    ...
    <div className="space-y-4 bg-slate-900/90 p-4 sm:p-5 rounded-2xl border border-slate-800">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 p-3 bg-slate-950 rounded-xl border border-slate-800 text-xs">
        <div className="flex items-center gap-2">
          ...
          <strong className="text-white font-semibold">{formatBRL(fipePrice)}</strong>
        </div>

        {fipeDifference < 0 ? (
          <span className="px-2.5 py-1 rounded-lg text-xs font-semibold bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 flex items-center gap-1">
            <TrendingDown className="w-3.5 h-3.5" />
            {formatBRL(Math.abs(fipeDifference))} abaixo da FIPE ({Math.abs(fipeDifferencePercent)}%)
          </span>
        ) : fipeDifference === 0 ? (
          <span className="px-2.5 py-1 rounded-lg text-xs font-semibold bg-blue-500/20 text-blue-300 border border-blue-500/30">
            No valor exato da FIPE
          </span>
        ) : (
          <span className="px-2.5 py-1 rounded-lg text-xs font-semibold bg-amber-500/20 text-amber-300 border border-amber-500/30 flex items-center gap-1">
      ...
      <div className="relative pt-6 pb-2">
        ...
        >
          ...
          <div className="w-4 h-4 rounded-full border-2 border-white bg-brand-500 shadow-md" />
        </div>
      </div>
