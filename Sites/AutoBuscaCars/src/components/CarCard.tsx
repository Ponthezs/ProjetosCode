...
export default function CarCard({ ad, compact = false }: CarCardProps) {
  ...
  return (
    <div className="group bg-slate-900 hover:bg-slate-800/60 border border-slate-800 hover:border-slate-700 rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-200 flex flex-col justify-between">
      ...
      <div className="relative aspect-[16/10] w-full bg-slate-950 overflow-hidden">
        ...
        {/* Rating score overlay bottom right */}
        <div className="absolute bottom-3 right-3 bg-slate-950/90 border border-slate-700 text-white px-2.5 py-1 rounded-xl text-xs font-bold flex items-center gap-1 shadow-md">
      ...
      <div className="p-4 sm:p-5 flex-1 flex flex-col justify-between space-y-3">
        <div>
          ...
          <Link href={`/carro/${ad.id}`} className="block group-hover:text-brand-400 transition-colors">
            <h3 className="font-semibold text-base sm:text-lg text-slate-100 line-clamp-1">
              {ad.brand} {ad.model}
            </h3>
        ...
        <div className="grid grid-cols-2 gap-2 pt-1">
          ...
          <a
            ...
            className="flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl text-xs font-semibold bg-brand-600 hover:bg-brand-500 text-white transition-all shadow-md shadow-brand-600/20"
